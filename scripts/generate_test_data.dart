...
class TestDataGenerator {
  ...
  static Future<void> main() async {
    ...
    // Save to JSON file
    const outputPath = 'c:\\Users\\Administrator\\genesis_f1\\test_data.json';
...
void main() async {
  ...
  final List<String> _hobbyTopics = [
    'skateboarding', 'guitar', 'music', 'football', 'soccer', 'basketball',
    'gaming', 'movies', 'tv shows', 'anime', 'books', 'photography',
    'cooking', 'travel', 'hiking', 'cycling', 'gym', 'fitness',
    'friends', 'family', 'relationships', 'dating', 'social life',
    'concerts', 'festivals', 'parties', 'hanging out'
  ];
  
  final List<String> _randomTopics = [
    'weather', 'food', 'coffee', 'sleep', 'dreams', 'thoughts',
    'random observations', 'daily life', 'routine', 'habits',
    'goals', 'plans', 'reflections', 'memories', 'nostalgia',
    'future', 'career', 'money', 'stress', 'relaxation'
  ];
  ...
  final List<String> _malaySlang = [
    'la', 'lah', 'bro', 'wei', 'woi', 'kan', 'jer', 'gak', 'tak',
    'sudah', 'belum', 'lagi', 'pun', 'pon', 'saje', 'aja'
  ];
  
  TestDataGenerator() {
    _endDate = DateTime.now();
    _startDate = _endDate.subtract(const Duration(days: 730)); // 2 years ago
  }
  
  Future<List<JournalEntry>> generateTwoYearJournal() async {
    print('📅 Generating entries from ${_startDate.toLocal()} to ${_endDate.toLocal()}');
    
    final entries = <JournalEntry>[];
    final currentDate = DateTime(_startDate.year, _startDate.month, _startDate.day);
    
    // Generate entries with varying frequency
    for (int day = 0; day < 730; day++) {
      final date = currentDate.add(Duration(days: day));
      
      // Determine how many entries for this day (0-3, weighted towards 0-1)
      final entryCount = _getEntriesForDay(date);
      
      for (int i = 0; i < entryCount; i++) {
        final entry = _generateEntryForDate(date, i);
        entries.add(entry);
      }
    }
    
    // Sort by date (oldest first for database insertion)
    entries.sort((a, b) => a.rawDateTime.compareTo(b.rawDateTime));
    
    return entries;
  }
  
  int _getEntriesForDay(DateTime date) {
    // More entries during:
    // - Weekends (slightly more social activities)
    // - University semester periods (more study stress)
    // - Holiday periods (more activities)
    
    final isWeekend = date.weekday >= 6;
    final isHoliday = _isHolidayPeriod(date);
    final isSemester = _isSemesterPeriod(date);
    
    // Base probability of having entries
    double entryProbability = 0.3; // 30% chance of any entry
    
    if (isWeekend) entryProbability += 0.2;
    if (isHoliday) entryProbability += 0.3;
    if (isSemester) entryProbability += 0.2;
    
    // Random factor
    entryProbability += (_random.nextDouble() - 0.5) * 0.3;
    entryProbability = entryProbability.clamp(0.0, 0.8);
    
    if (_random.nextDouble() > entryProbability) return 0;
    
    // Determine number of entries (1-3, weighted towards 1)
    final rand = _random.nextDouble();
  ...
  bool _isHolidayPeriod(DateTime date) {
    // Malaysian university holidays and common break periods
    final month = date.month;
    return month == 12 || month == 1 || // Year-end holidays
           (month >= 6 && month <= 8) || // Mid-year break
           month == 3; // March break
  }
  
  bool _isSemesterPeriod(DateTime date) {
    // Active semester periods with more studying
    final month = date.month;
    return (month >= 2 && month <= 5) || // Semester 1
           (month >= 9 && month <= 11);   // Semester 2
  }
  
  JournalEntry _generateEntryForDate(DateTime date, int entryIndex) {
    // Generate time for the entry
    final hour = _generateRealisticHour(entryIndex);
    final minute = _random.nextInt(60);
    final entryDateTime = DateTime(date.year, date.month, date.day, hour, minute);
    
    // Determine entry type and content
    final entryType = _selectEntryType(date);
    final content = _generateContent(entryType, date);
    
    // Generate tags and mood
    final tags = _generateTags(entryType, content);
    final mood = _random.nextDouble() < 0.4 ? _moods[_random.nextInt(_moods.length)] : null;
    
    // Calculate word count
    final wordCount = content.split(RegExp(r'\s+')).where((w) => w.isNotEmpty).length;
    
    // Generate IDs
    final localId = 'local_${entryDateTime.millisecondsSinceEpoch}_${_random.nextInt(1000)}';
    
    return JournalEntry(
      localId: localId,
      text: content,
      timestamp: _formatTime(entryDateTime),
      rawDateTime: entryDateTime,
      mood: mood,
      tags: tags,
      wordCount: wordCount,
      isFavorite: _random.nextDouble() < 0.1, // 10% chance of being favorite
      isSynced: false,
    );
  }
  
  int _generateRealisticHour(int entryIndex) {
    // Different times of day based on entry index and realistic patterns
    if (entryIndex == 0) {
      // First entry: morning or evening
      return _random.nextBool() ? 
        _random.nextInt(5) + 7 : // 7-11 AM
        _random.nextInt(4) + 19; // 7-10 PM
    } else if (entryIndex == 1) {
      // Second entry: afternoon or late night
      return _random.nextBool() ? 
        _random.nextInt(4) + 14 : // 2-5 PM
        _random.nextInt(3) + 22; // 10 PM-12 AM
    } else {
      // Third entry: late night
      return _random.nextInt(3) + 23; // 11 PM-1 AM (next day)
    }
  }
  
  String _selectEntryType(DateTime date) {
    final isWeekend = date.weekday >= 6;
    final isSemester = _isSemesterPeriod(date);
    
    final types = <String, double>{
      'software_engineering': isSemester ? 0.3 : 0.15,
      'part_time_job': 0.2,
      'hobby': isWeekend ? 0.4 : 0.25,
      'random': 0.3,
      'lazy': 0.15,
    };
    
    return _weightedRandomChoice(types);
  }
  
  String _weightedRandomChoice(Map<String, double> weights) {
  ...
  String _generateContent(String entryType, DateTime date) {
    switch (entryType) {
      case 'software_engineering':
        return _generateSoftwareEngineeringEntry();
      case 'part_time_job':
        return _generatePartTimeJobEntry();
      case 'hobby':
        return _generateHobbyEntry();
      case 'lazy':
        return _generateLazyEntry();
      default:
        return _generateRandomEntry();
    }
  }
  
  String _generateSoftwareEngineeringEntry() {
    final topics = _sampleList(_softwareEngineeringTopics, _random.nextInt(3) + 1);
    final templates = [
      'Today learned about {topic}. Still struggling with {problem} but making progress lah.',
      'Assignment on {topic} due tomorrow. Procrastinated again smh. Time to grind.',
      'Finally understood {topic}! Professor explained it better today. Feeling confident for exam.',
      'Group project meeting about {topic}. Some teammates not pulling their weight wei.',
      'Debugging session lasted 3 hours. Found the bug in {topic} implementation. So satisfying!',
      'Lecture on {topic} was confusing. Need to review slides and practice more.',
      'Coding interview prep on {topic}. Leetcode is destroying my soul but gotta push through.',
      'Lab session on {topic} went well. Partner actually knew what they were doing for once.',
      'Struggling with {topic} concepts. Maybe should ask TA for help during office hours.',
      'Final year project progress on {topic}. Supervisor wants more documentation ugh.',
    ];
    
    return _fillTemplate(templates, {'topic': topics.first, 'problem': topics.length > 1 ? topics[1] : 'implementation'});
  }
  
  String _generatePartTimeJobEntry() {
    final jobs = _sampleList(_partTimeJobTopics, 1);
    final templates = [
      'Work shift at {job} today. Customers were extra demanding lah. Need the money though.',
      '{job} training session was boring but necessary. At least got paid to sit there.',
      'Supervisor at {job} being unreasonable again. Looking for other opportunities.',
      'Good day at {job}! Tips were decent and coworkers were chill.',
      'Called in sick for {job} because assignment deadline. Hope they don\'t fire me.',
      'Overtime at {job} because someone didn\'t show up. Extra cash but exhausted now.',
      'Thinking of quitting {job}. Scheduling conflicts with classes too much.',
      'New coworker at {job} is pretty cool. Makes shifts more bearable.',
      'Busy day at {job}. Feet killing me but hustle doesn\'t stop.',
      'Got employee of the month at {job}! Small victory but still counts.',
    ];
    
    return _fillTemplate(templates, {'job': jobs.first});
  }
  
  String _generateHobbyEntry() {
    final hobbies = _sampleList(_hobbyTopics, _random.nextInt(2) + 1);
    final templates = [
      'Went {hobby} with the gang today. So good to unwind after stressful week.',
      'New {hobby} trick/technique learned! Progress feels slow but steady lah.',
      'Equipment for {hobby} broke down. Need to save up for replacement.',
      'Competition/game of {hobby} tomorrow. Been practicing but still nervous.',
      'Weather perfect for {hobby} today. Made the most of it with friends.',
      'Discovered new spot for {hobby}. Hidden gem that not many people know about.',
      'Teaching friend how to {hobby}. Harder than expected to explain basics.',
      'Injured myself doing {hobby}. Nothing serious but need to be more careful.',
      'Video/tutorial on {hobby} was super helpful. Trying new approaches now.',
      'Long session of {hobby} today. Lost track of time but felt so relaxed.',
      'Group meetup for {hobby} was epic. Community is really supportive here.',
    ];
    
    return _fillTemplate(templates, {'hobby': hobbies.first});
  }
  
  String _generateLazyEntry() {
    final templates = [
      'whole day just netflix and chill. no regrets.',
      'supposed to study but ended up scrolling tiktok for 4 hours lol',
      'ordered grabfood again. wallet crying but too lazy to cook',
      'woke up at 2pm. schedule completely messed up but whatever',
      'roommate judging my lifestyle choices. not my fault online classes make me nocturnal',
      'planned to be productive today. narrator: he was not productive',
      'coffee shop wifi down so used it as excuse to not do work',
      'existential crisis at 3am again. why am i like this',
      'started 5 different shows today. attention span of a goldfish',
      'cleaned room for first time in weeks. feeling oddly accomplished',
    ];
    
    return templates[_random.nextInt(templates.length)];
  }
  
  String _generateRandomEntry() {
    final topics = _sampleList(_randomTopics, 1);
    final templates = [
      'Random thought about {topic}. Mind works in mysterious ways.',
      'Weird dream involving {topic} last night. Subconscious is wild.',
      'Conversation about {topic} with friend got unexpectedly deep.',
      '{topic} observation from today. Small things matter sometimes.',
      'Reminded of {topic} from childhood. Nostalgia hitting different.',
      'Social media post about {topic} made me think. Perspective shift.',
      'Podcast mentioned {topic}. Need to research more about this.',
      'Family discussion about {topic} during dinner. Generational differences.',
      'Article on {topic} was eye-opening. Sharing with friends later.',
      'Personal reflection on {topic}. Growth mindset kicking in.',
    ];
    
    return _fillTemplate(templates, {'topic': topics.first});
  }
  
  String _fillTemplate(List<String> templates, Map<String, String> replacements) {
    ...
    // Add Malaysian slang randomly
    if (_random.nextDouble() < 0.3) {
      final slang = _malaySlang[_random.nextInt(_malaySlang.length)];
      template += ' $slang';
    }
    
    // Add typos and shortforms occasionally
    if (_random.nextDouble() < 0.2) {
      template = _addTyposAndShortforms(template);
    }
    
    return template;
  }
  
  String _addTyposAndShortforms(String text) {
    final replacements = {
      'and': 'n',
      'you': 'u',
      'your': 'ur',
    ...
    replacements.forEach((from, to) {
      if (_random.nextDouble() < 0.3) { // 30% chance to replace
        result = result.replaceAll(RegExp('\\b$from\\b', caseSensitive: false), to);
      }
    });
  ...
  List<String> _generateTags(String entryType, String content) {
    final tags = <String>[];
    
    // Add type-based tags
    switch (entryType) {
      case 'software_engineering':
        tags.addAll(['study', 'university', 'coding']);
        break;
      case 'part_time_job':
        tags.addAll(['work', 'money', 'job']);
        break;
      case 'hobby':
        tags.addAll(['hobby', 'fun', 'friends']);
        break;
      case 'lazy':
        tags.addAll(['lazy', 'procrastination']);
        break;
      default:
        tags.addAll(['thoughts', 'daily']);
    }
    
    // Add content-based tags
    final contentLower = content.toLowerCase();
    final possibleTags = [
      'stress', 'happy', 'tired', 'excited', 'frustrated', 'motivated',
      'family', 'friends', 'food', 'music', 'gaming', 'exercise',
      'money', 'health', 'sleep', 'weather', 'travel', 'goals'
    ];
    
    for (final tag in possibleTags) {
      if (contentLower.contains(tag) && _random.nextDouble() < 0.4) {
        tags.add(tag);
      }
    }
    
    // Limit tags and shuffle
    tags.shuffle();
    return tags.take(5).toList();
  }
  
  List<T> _sampleList<T>(List<T> list, int count) {
  ...
  Map<String, dynamic> getStatistics(List<JournalEntry> entries) {
    final stats = <String, dynamic>{};
    
    stats['Total entries'] = entries.length;
    stats['Date range'] = '${_startDate.toLocal().toString().split(' ')[0]} to ${_endDate.toLocal().toString().split(' ')[0]}';
    stats['Average per day'] = (entries.length / 730).toStringAsFixed(1);
    stats['Entries with mood'] = entries.where((e) => e.mood != null).length;
    stats['Favorite entries'] = entries.where((e) => e.isFavorite).length;
    stats['Total words'] = entries.fold(0, (sum, e) => sum + e.wordCount);
    stats['Average words per entry'] = (entries.fold(0, (sum, e) => sum + e.wordCount) / entries.length).toStringAsFixed(0);
    ...
    final topTags = tagCounts.entries.toList()
      ..sort((a, b) => b.value.compareTo(a.value));
    stats['Top 5 tags'] = topTags.take(5).map((e) => '${e.key} (${e.value})').join(', ');
...
class JournalEntry {
  ...
  Map<String, dynamic> toJson() {
    return {
      'localId': localId,
      'text': text,
      'timestamp': rawDateTime.toIso8601String(),
      'mood': mood,
      'tags': tags,
      'wordCount': wordCount,
      'isFavorite': isFavorite,
      'isSynced': isSynced,
      // Note: imageUrl, localImagePath will be null for this generated data
      'imageUrl': null,
      'localImagePath': null,
    };
  }
